...
public class Commands implements CommandExecutor {
    ...
    public boolean delete(final Player player, final String[] args) {
        if (args.length == 3) {
            ...
            this.isSuccess = Io.delete(Path.of(world));
            if (this.isSuccess) {
    ...
    public boolean duplicate(final Player player, final String[] args) {
        ...
        Bukkit.getScheduler().runTask(this.plugin, () -> {
            ...
            Commands.this.isSuccess = Io.copy(Path.of(world), Path.of(newWorld));
            if (Commands.this.isSuccess) {
                Io.delete(Path.of(newWorld, "uid.dat"));
                load(newWorld);
                if (hasMultiverse()) {
        ...
        if ((args.length > 0 && player != null) || (player == null && args.length > 1)) {
            Bukkit.getScheduler().runTask(this.plugin, () -> {
                ...
                if (isDefault) {
                    ...
                    unload(world, true);
                    if (args[0].equalsIgnoreCase("save")) {
                        Commands.this.isSuccess = Io.copy(Path.of(world), Path.of(backup));
                    } else {
                        Commands.this.isSuccess = Io.copy(Path.of(backup), Path.of(world));
                    }
